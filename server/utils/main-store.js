...
export async function refreshMainFromDb() {
  try {
    const row = await prisma.main.findFirst({ orderBy: { updatedAt: 'desc' } })
    data = row?.data ?? null
...
export function startMainRefresh() {
  setInterval(refreshMainFromDb, INTERVAL_MS)
}
